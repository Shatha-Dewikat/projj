...
window.addEventListener("scroll", function() {
    ...
    const serviceItems = document.querySelectorAll('.Wfirst');
    serviceItems.forEach(item=>{
        if (window.scrollY  <= item.offsetTop+10*item.offsetTop) {
            item.style.animation = 'slideside 3s ease forwards';
        }
    });

    const serviceItems2 = document.querySelectorAll('.Wsec');
    serviceItems2.forEach(item=>{
        if (window.scrollY  <= item.offsetTop+10*item.offsetTop) {
            item.style.animation = 'slideside2 3s ease forwards';
        }
    });
